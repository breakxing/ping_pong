...
inline void set_ini(char *path)
{
    ...
    assert(chunk_segment != 0);
    assert(file_segment == thread_segment);
    // assert (N <= MAX_QUBIT);
...
void set_state_files()
{
    ...
    free(state_dir);
    #if profiling
        char *pro_dir = (char *)malloc(max_path * sizeof(char));
        strcpy(pro_dir, "./profile");
        mk_dir(pro_dir);
        free(pro_dir);
    #endif
...
#pragma omp parallel for num_threads(num_thread) schedule(static, 1)
    ...
    {
        #if profiling
            char profile_file_name[max_path];
            sprintf(profile_file_name, "./profile/thread%d.txt", t);
            thread_settings[t].file = fopen(profile_file_name, "w");
        #endif
...
inline int read_args(int argc, char *argv[], char **ini, char **cir)
{
    ...
    while (1)
    {
        ...
        case 'c':
            ...
            *cir = (char *)malloc(sizeof(char) * (destination_size + 1));
        ...
        case 'i':
            ...
            destination_size = strlen(optarg);
            *ini = (char *)malloc(sizeof(char) * (destination_size + 1));
